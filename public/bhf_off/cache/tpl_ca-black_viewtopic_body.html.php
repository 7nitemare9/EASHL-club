...
<!-- NOTE: remove the style="display: none" when you want to have the forum description on the topic body --><?php if ($this->_rootref['FORUM_DESC']) {  ?><p class="forum-desc" style="display: none;"><?php echo (isset($this->_rootref['FORUM_DESC'])) ? $this->_rootref['FORUM_DESC'] : ''; ?></p><?php } if ($this->_rootref['MODERATORS'] || $this->_rootref['U_MCP']) {  ?>
	...
	<div class="panel" id="poll">
		...
		<div class="content">
			...
			<?php $_poll_option_count = (isset($this->_tpldata['poll_option'])) ? sizeof($this->_tpldata['poll_option']) : 0;if ($_poll_option_count) {for ($_poll_option_i = 0; $_poll_option_i < $_poll_option_count; ++$_poll_option_i){$_poll_option_val = &$this->_tpldata['poll_option'][$_poll_option_i]; ?>
				...
				<dl class="poll-row<?php if ($_poll_option_val['POLL_OPTION_VOTED']) {  ?> voted<?php } if ($_poll_option_val['S_FIRST_ROW']) {  ?> poll-first<?php } ?>"<?php if ($_poll_option_val['POLL_OPTION_VOTED']) {  ?> title="<?php echo ((isset($this->_rootref['L_POLL_VOTED_OPTION'])) ? $this->_rootref['L_POLL_VOTED_OPTION'] : ((isset($user->lang['POLL_VOTED_OPTION'])) ? $user->lang['POLL_VOTED_OPTION'] : '{ POLL_VOTED_OPTION }')); ?>"<?php } ?>>
...
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } ?>
	...
	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post<?php if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } ?>">
		<?php if ($_postrow_val['S_ROW_COUNT'] == 0) {  echo (isset($this->_tpldata['DEFINE']['.']['CA_POST_FIRST_START'])) ? $this->_tpldata['DEFINE']['.']['CA_POST_FIRST_START'] : ''; } else { echo (isset($this->_tpldata['DEFINE']['.']['CA_POST_START'])) ? $this->_tpldata['DEFINE']['.']['CA_POST_START'] : ''; } if (! $_postrow_val['S_IGNORE_POST']) {  $this->_tpldata['DEFINE']['.']['CA_POPUP'] = '0'; if (! $this->_rootref['S_IS_BOT']) {  if ($_postrow_val['U_PM'] || $_postrow_val['U_EMAIL'] || $_postrow_val['U_WWW'] || $_postrow_val['U_MSN'] || $_postrow_val['U_ICQ'] || $_postrow_val['U_YIM'] || $_postrow_val['U_AIM'] || $_postrow_val['U_JABBER']) {  $this->_tpldata['DEFINE']['.']['CA_POPUP'] = '1'; } } ?>

        <div id="maximize<?php echo $_postrow_val['S_ROW_COUNT']; ?>" class="post-maximize" style="display: none;">
            <a href="javascript:void(0);" onclick="ca_post_maximize_all(); return false;"><span>&gt;&gt;</span></a>
        </div>
        <div id="profilediv<?php echo $_postrow_val['S_ROW_COUNT']; ?>">
        <?php echo (isset($this->_tpldata['DEFINE']['.']['CA_POST_PROFILE_START'])) ? $this->_tpldata['DEFINE']['.']['CA_POST_PROFILE_START'] : ''; ?>

        <div class="postprofile" id="profile<?php echo $_postrow_val['S_ROW_COUNT']; ?>">
            <div id="minimize<?php echo $_postrow_val['S_ROW_COUNT']; ?>" class="post-minimize">
                <a href="javascript:void(0);" onclick="ca_post_minimize_all(); return false;"><span>&lt;&lt;</span></a>
            </div>
            <div class="profile-top">
                <p class="profile-author">
                    <a class="profile-link" id="link<?php echo $_postrow_val['S_ROW_COUNT']; ?>" href="<?php if ($_postrow_val['U_POST_AUTHOR']) {  echo $_postrow_val['U_POST_AUTHOR']; } else { ?>javascript:void(0);<?php } ?>"<?php if ($_postrow_val['POST_AUTHOR_COLOUR']) {  ?> style="color: <?php echo $_postrow_val['POST_AUTHOR_COLOUR']; ?>;"<?php } ?>><?php echo $_postrow_val['POST_AUTHOR']; ?></a>
                </p>
                <?php if ($this->_tpldata['DEFINE']['.']['CA_POPUP']) {  ?>

                    <div id="popup<?php echo $_postrow_val['S_ROW_COUNT']; ?>" class="profile-buttons">
                        <ul>
                            <li><a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>"><?php echo ((isset($this->_rootref['L_READ_PROFILE'])) ? $this->_rootref['L_READ_PROFILE'] : ((isset($user->lang['READ_PROFILE'])) ? $user->lang['READ_PROFILE'] : '{ READ_PROFILE }')); ?></a></li>
                ...
                // <![CDATA[
                    ca_popup_register('<?php echo $_postrow_val['S_ROW_COUNT']; ?>');
                // ]]>
                </script>
            </div>
            ...
            <div class="profile-avatar">
                <?php echo $_postrow_val['POSTER_AVATAR']; ?>

            </div>    
            <?php } if ($_postrow_val['RANK_TITLE'] || $_postrow_val['RANK_IMG']) {  ?>

			    <p class="profile-rank">
			        <?php echo $_postrow_val['RANK_TITLE']; ?>

			        <?php if ($_postrow_val['RANK_TITLE'] && $_postrow_val['RANK_IMG']) {  ?><br /><?php } ?>
            ...
            <div class="profile-details">
                <?php if ($_postrow_val['POSTER_POSTS'] != ('')) {  ?><p><strong><?php echo ((isset($this->_rootref['L_POSTS'])) ? $this->_rootref['L_POSTS'] : ((isset($user->lang['POSTS'])) ? $user->lang['POSTS'] : '{ POSTS }')); ?>:</strong> <?php echo $_postrow_val['POSTER_POSTS']; ?></p><?php } if ($_postrow_val['POSTER_JOINED']) {  ?><p><strong><?php echo ((isset($this->_rootref['L_JOINED'])) ? $this->_rootref['L_JOINED'] : ((isset($user->lang['JOINED'])) ? $user->lang['JOINED'] : '{ JOINED }')); ?>:</strong> <?php echo $_postrow_val['POSTER_JOINED']; ?></p><?php } if ($_postrow_val['POSTER_FROM']) {  ?><p><strong><?php echo ((isset($this->_rootref['L_LOCATION'])) ? $this->_rootref['L_LOCATION'] : ((isset($user->lang['LOCATION'])) ? $user->lang['LOCATION'] : '{ LOCATION }')); ?>:</strong> <?php echo $_postrow_val['POSTER_FROM']; ?></p><?php } if ($_postrow_val['S_PROFILE_FIELD1']) {  ?><!-- Use a construct like this to include admin defined profile fields. Replace FIELD1 with the name of your field. -->
                    <p><strong><?php echo $_postrow_val['PROFILE_FIELD1_NAME']; ?>:</strong> <?php echo $_postrow_val['PROFILE_FIELD1_VALUE']; ?></p>
                <?php } $_custom_fields_count = (isset($_postrow_val['custom_fields'])) ? sizeof($_postrow_val['custom_fields']) : 0;if ($_custom_fields_count) {for ($_custom_fields_i = 0; $_custom_fields_i < $_custom_fields_count; ++$_custom_fields_i){$_custom_fields_val = &$_postrow_val['custom_fields'][$_custom_fields_i]; ?>
                    ...
                    <p><strong><?php echo $_custom_fields_val['PROFILE_FIELD_NAME']; ?>:</strong> <?php echo $_custom_fields_val['PROFILE_FIELD_VALUE']; ?></p>
                <?php }} ?>

            </div>
		</div>
		<?php echo (isset($this->_tpldata['DEFINE']['.']['CA_POST_PROFILE_END'])) ? $this->_tpldata['DEFINE']['.']['CA_POST_PROFILE_END'] : ''; ?>
		...
		<div class="postbody" id="body<?php echo $_postrow_val['S_ROW_COUNT']; ?>">
			...
			<p class="author"><?php if ($this->_rootref['S_IS_BOT']) {  echo $_postrow_val['MINI_POST_IMG']; } else { ?><a href="<?php echo $_postrow_val['U_MINI_POST']; ?>"><?php echo $_postrow_val['MINI_POST_IMG']; ?></a><?php } ?> <?php echo $_postrow_val['POST_DATE']; ?> <?php if ($_postrow_val['POST_AUTHOR_FULL']) {  ?><span id="author<?php echo $_postrow_val['S_ROW_COUNT']; ?>" style="display: none;"><?php echo ((isset($this->_rootref['L_POST_BY_AUTHOR'])) ? $this->_rootref['L_POST_BY_AUTHOR'] : ((isset($user->lang['POST_BY_AUTHOR'])) ? $user->lang['POST_BY_AUTHOR'] : '{ POST_BY_AUTHOR }')); ?> <?php echo $_postrow_val['POST_AUTHOR_FULL']; ?></span><?php } ?></p>
